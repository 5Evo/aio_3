...
class Logger:
    ...
    def __init__(self, logging_service=LOGGING_SERVICE, enable_notifications=True):
        ...
        # Logger setup:
        self.Logger = logging.getLogger(f"{logging_service}_logger")    # создание объекта логгера с именем "{logging_service}_logger"
        ...
        formatter = logging.Formatter("%(asctime)s - %(levelname)s - %(name)s - %(message)s")   # создание объекта форматирования логов
        log_path = os.path.join(ROOT_DIR, LOG_PATH)
        log_file = f"{log_path}/{logging_service}.log"
        fh = logging.FileHandler(log_file)     # создание объекта обработчика файла, который записывает логи в файл с именем "{logging_service}.log"
